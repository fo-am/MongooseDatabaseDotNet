...
namespace psDataImporter.Data
{
    public class PostgresRepository
    {
        ...
        public void AddFoetus(int pack_history_id, string foetusNumber, int foetusId, DateTime ultrasoundDate, string foetusSize,
            ...
            string comment, string observer)
        {
            ...
            {
                conn.Execute(
                    "Insert into mongoose.ultrasound (pack_history_id, observation_date, foetus_number, foetus_id, foetus_size, cross_view_length, cross_view_width, long_view_length, long_view_width, observer, comment)" +
                    " values(@pack_history_id, @observationDate, @foetusNumber, @foetus_id, @foetusSize, @crossViewLength, @crossViewWidth, @longViewLength, @longViewWidth, @Observer, @comment)",
                    new
                    {
                        pack_history_id,
        ...
        public void LinkIndividualEvents(int pack_history_id, int individualEventCodeId, string latitude,
            string longitude,
            string startend,
            string status, DateTime date, string exact, string cause, string comment, int? affectedLitter)
        {
            // create geography if lat and long are present.
            var locationString = LocationString(latitude, longitude);

            var sql =
                "Insert into mongoose.individual_event (pack_history_id, individual_event_code_id, status, date, exact, start_end, cause, affected_litter, comment, location )" +
                $" values (@pack_history_id, @individualEventCodeId, @status, @date, @exact, @start_end, @cause, @affected_litter, @Comment, {locationString})";
                ...
                .ConnectionString))
            {
                ...
                conn.Execute(sql,
                    new
                    {
                        ...
                        exact,
                        cause,
                        affected_litter = affectedLitter,
                        comment
                    });
            }
        }
